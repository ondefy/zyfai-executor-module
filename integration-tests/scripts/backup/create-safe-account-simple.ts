...
/**
 * Create a simple Safe smart account without any registry hooks
 */
async function main() {
  console.log("🏗️ Creating simple Safe smart account without registry hooks...");
  ...
  // Create Safe account WITHOUT any attesters - this prevents registry hook installation
  const safeAccount = await toSafeSmartAccount({
    ...
    // NO ATTESTERS AT ALL - This prevents any registry hooks
    validators: [
      {
    ...
    saltNonce: fromHex(toHex(process.env.ACCOUNT_SALT || '0x3'), 'bigint'), // Different salt to create new account
  });
  ...
  console.log("\n✅ Simple Safe account created and saved to env.base");
  console.log("Safe Address:", safeAddress);
  
  console.log("\n📝 Next steps:");
  console.log("1. Fund the account with at least 0.001 ETH");
  console.log("2. Run: pnpm run install-module-safe-native");
