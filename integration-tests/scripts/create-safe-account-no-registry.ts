...
/**
 * Create a Safe smart account WITHOUT registry hook for custom modules
 */
async function main() {
  console.log("🏗️ Creating Safe smart account WITHOUT registry hook...");
  ...
  console.log("\n🔧 Creating Safe account WITHOUT registry hook...");
  ...
  // Create Safe account WITHOUT registry hook by not including attesters
  const safeAccount = await toSafeSmartAccount({
    ...
    // NO ATTESTERS - This prevents registry hook installation
    // Remove attesters completely instead of empty array
    validators: [
      {
    ...
    saltNonce: fromHex(toHex(process.env.ACCOUNT_SALT || '0x2'), 'bigint'), // Different salt to create new account
  });
  ...
  if (balance < parseEther("0.001")) {
    console.log("⚠️ Safe account needs funding. Please fund it manually.");
    console.log("Send at least 0.001 ETH to:", safeAddress);
  } else {
  ...
  console.log("\n✅ Safe account created WITHOUT registry hook and saved to env.base");
  console.log("Safe Address:", safeAddress);
  
  console.log("\n📝 Next steps:");
  console.log("1. Fund the account with at least 0.001 ETH");
  console.log("2. Run: pnpm run install-module");
  console.log("3. Run: pnpm run setup-whitelist");
  console.log("4. Run: pnpm run create-session-key");
  console.log("5. Run: pnpm run test-integration");
}
